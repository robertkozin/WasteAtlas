  ...
  loader: directusLoader({
    command: lastModified =>
      readItems("waste", {
        ...
        filter: {
          slug: { _nnull: true },
          _or: [{ date_created: { _gte: lastModified } }, { date_updated: { _gte: lastModified } }],
        },
        sort: ["name", "location"],
      }),
    map: (waste, idx) => {
  ...
  loader: directusLoader({
    ...
    map: project => {
      return {
        ...
        image_urls: project.images.map(img => getAssetUrl(img.directus_files_id)),
  ...
  loader: directusLoader({
    command: lastModified =>
      readFiles({
        ...
        filter: {
          _or: [{ created_on: { _gte: lastModified } }, { modified_on: { _gte: lastModified } }],
        },
      }),
    map: img => ({
      ...img,
